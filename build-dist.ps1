...
$need = @(
  ...
  "$ROOT\server\trace-image.js",   # 2026-09-22: 位图→CAD 描摹引擎(llm-agent/file-tools 依赖, 漏打包 traceImage 直接报模块找不到)
  ...
  "$ROOT\tools\locale.ps1",  # 2026-09-21
  "$ROOT\knowledge\image-to-cad.md"   # 2026-09-22: 图片→CAD 分诊 SOP 引用的知识库文档   # 2026-09-21: 脚本共用语言助手(port-check/port-config/clean-runtime 依赖)
)
foreach ($f in $need) {
...
Copy-Item "$ROOT\tools\clean-runtime.ps1" "$DIST\tools\" -Force
  Copy-Item "$ROOT\tools\locale.ps1" "$DIST\tools\" -Force
...
# ---------- 4. 使用说明 ----------
$readme = @"
new-acad v$VERSION — Civil 3D AI assistant（portable / 绿色版）
==============================================================
