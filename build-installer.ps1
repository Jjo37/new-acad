...
[Run]
Filename: "powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -File ""{app}\install.ps1"""; WorkingDir: "{app}"; StatusMsg: "运行环境配置向导（检查 C3D/注册自启/配置 AI）..."; Flags: postinstall skipifsilent; Description: "运行环境配置向导（推荐：检查 C3D + 注册自启 + 配置 AI）"
