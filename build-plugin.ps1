...
try {
  ...
  # 2026-09-22: 同步到 Autoloader bundle —— C3D 实际 NETLOAD 的是这份（漏了会出现"新方法不生效"）
  $bundleWin = Join-Path $env:APPDATA "Autodesk\ApplicationPlugins\new-acad.bundle\Contents\Windows"
  if (Test-Path $bundleWin) {
    $bTarget = Join-Path $bundleWin "Civil3DMcpPlugin.dll"
    try {
      Copy-Item $bTarget "$bTarget.bak" -Force -ErrorAction SilentlyContinue
      Copy-Item "..\Civil3DMcpPlugin.dll" $bTarget -Force
      Copy-Item "..\Civil3DMcpPlugin.deps.json" $bundleWin -Force -ErrorAction SilentlyContinue
      Write-Host "[OK] synced to autoloader bundle: $bundleWin" -ForegroundColor Green
    } catch {
      Write-Host "[WARN] bundle sync failed (C3D still running?): $($_.Exception.Message)" -ForegroundColor Yellow
    }
  }
