...
const ACTION_MAP = {
  ...
  createHatch: '创建填充', importHatches: '批量创建填充', editHatch: '编辑填充', importVectorPaths: '批量导入矢量路径',
...
- ⚠️ **禁止编造插件限制**：任何"插件不支持 / 没有 / 不能"的结论，**必须先调 listMethods 求证**；确有缺失就报【MISSING】或说明需要新方法补充，**不许自己编限制、不许因此把活甩给用户**
- 🧹 **描摹/批量绘制后自清理**：重描或换参数前，先删自己上一版：getEntitiesByLayer{layer} → deleteEntities{handles} → deleteLayer{name}（均可用、可翻页取全量）；**不许让用户手动删图层**；最终只留选定方案那一层
