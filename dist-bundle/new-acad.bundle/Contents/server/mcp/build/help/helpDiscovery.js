...
export function discoverOfflineHelpInstallations(options = {}) {
    ...
    for (const installation of installations) {
        ...
        if (!unique.has(key) || installation.configured)
    ...
    return [...unique.values()].sort((left, right) => {
        ...
        if (configuredVersion) {
            if (left.version === configuredVersion && right.version !== configuredVersion)
                return -1;
            if (right.version === configuredVersion && left.version !== configuredVersion)
                return 1;
        }
        return Number(right.version) - Number(left.version) || left.language.localeCompare(right.language);
    });
}
export function selectHelpInstallation(installations, requestedVersion) {
...
export function assertPathInsideHelpRoot(root, candidate) {
...
function isHelpRoot(root) {
    if (!fs.existsSync(root) || !fs.statSync(root).isDirectory())
        return false;
    if (!fs.existsSync(path.join(root, "index.html")))
        return false;
    return fs.readdirSync(root, { withFileTypes: true }).some((entry) => entry.isDirectory() && (/^wrapped-files/i.test(entry.name) || /^files/i.test(entry.name)));
}
