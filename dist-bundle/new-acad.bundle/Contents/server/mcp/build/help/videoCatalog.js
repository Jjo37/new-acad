...
let defaultCatalog;
export function getAutodeskVideoCatalog() {
    defaultCatalog ??= AutodeskVideoCatalog.fromDefaultFile();
    return defaultCatalog;
}
export function renderVideoPlayerHtml(video) {
...
function trustedAutodeskUrl(value, requiredSuffix) {
    if (!value)
        return undefined;
    try {
        const url = new URL(value);
        if (url.protocol !== "https:" || url.hostname !== "help.autodesk.com")
            return undefined;
        if (!url.pathname.toLowerCase().endsWith(requiredSuffix))
            return undefined;
        return url.href;
    }
    catch {
