...
function parsePartDiameter(partName) {
...
function computeFullFlowCapacity(diameter, slopePct, manningsN) {
...
function computeVelocity(flow, diameter) {
    const area = Math.PI * diameter * diameter / 4;
    return area > 0 ? flow / area : 0;
}
function solveRequiredDiameter(flow, slopePct, manningsN) {
    let low = 0.01;
    let high = 100;
...
function chooseBestPart(partNames, requiredDiameter, flow, velocityMin, velocityMax) {
    const candidates = partNames
        .map((name) => ({ name, diameter: parsePartDiameter(name) }))
        .filter((candidate) => candidate.diameter != null)
...
export const PIPE_DOMAIN_DEFINITION = {
    domain: "pipe",
    actions: {
        list: {
            action: "list",
        ...
        size_network: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => {
                const network = await appClient.sendCommand("getPipeNetwork", { name: args.networkName });
                ...
                const catalog = await appClient.sendCommand("listPipePartsCatalog", { partsList: partsListName });
                ...
                const perPipeDesignFlows = (args.perPipeDesignFlows ?? []);
        ...
        automate_profile_view: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => {
                ...
                const profileList = await appClient.sendCommand("listProfiles", { alignmentName });
    ...
    exposures: [
        {
            ...
            description: "管网聚合工具：查询/读取/水力分析/碰撞检查",
        ...
        },
        {
            ...
            description: "重力管网聚合工具：创建/查询/加管加井/改尺寸",
            inputShape: {
        ...
        },
        {
            ...
            description: "管网水力选型 {networkName, ...}：按设计流量/曼宁系数/目标流速计算管径，可应用变更",
        ...
        },
        {
            ...
            description: "自动创建管网纵断面图 {networkName, profileViewName, ...}",
