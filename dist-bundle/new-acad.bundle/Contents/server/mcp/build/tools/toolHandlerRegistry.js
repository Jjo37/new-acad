 ...
 */
export function captureToolHandler(name, handler) {
 ...
 */
export async function executeRegisteredTool(toolName, parameters) {
    const handler = _handlers.get(toolName);
    if (!handler) {
        throw new Error(`Tool '${toolName}' is not registered. Available: ${listRegisteredToolNames().length} tools.`);
    }
    const mcpResult = await handler(parameters, {});
    // If the tool signalled an error, throw so the caller can set the HTTP status
    if (mcpResult?.isError) {
        const errorText = mcpResult.content?.[0]?.text ?? `Tool '${toolName}' returned an error`;
        ...
        throw new Civil3DRpcError(errorText, mcpResult.errorCode, mcpResult.rpcCode ?? -32000);
    }
    // Prefer the typed MCP result while preserving the plain HTTP payload shape.
    if (mcpResult?.structuredContent) {
        if ("result" in mcpResult.structuredContent &&
