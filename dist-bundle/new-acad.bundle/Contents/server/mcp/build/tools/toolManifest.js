import { buildDomainToolCatalogEntries, captureDomainToolHandlers, registerSelectedDomainTools, } from "./domainRuntime.js";
...
import { HANK_DOMAIN_DEFINITION } from "./domains/hankDomain.js";
// hank 是 @ts-nocheck 保留的本地定制，domain 字段为 string，断言为 DomainToolDefinition
const HANK_DOMAIN = HANK_DOMAIN_DEFINITION;
...
export const GENERATED_TOOL_CATALOG_ENTRIES = MIGRATED_DOMAIN_DEFINITIONS.flatMap((definition) => buildDomainToolCatalogEntries(definition));
export function findManifestAction(toolName, action) {
    for (const definition of MIGRATED_DOMAIN_DEFINITIONS) {
        const exposure = definition.exposures.find((candidate) => candidate.toolName === toolName && candidate.supportedActions.includes(action));
...
export function registerManifestTools(server) {
...
export function selectManifestExposures(definition, includeAliases = false) {
    if (includeAliases) {
    ...
    if (definition.domain === "docs") {
        const orchestrator = definition.exposures.find((exposure) => exposure.toolName === "civil3d_orchestrate");
