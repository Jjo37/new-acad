...
2. 调用 C3D 插件：HTTP POST http://127.0.0.1:19876/tcp，body 为 JSON-RPC：{"jsonrpc":"2.0","id":1,"method":"方法名","params":{...}}
   ...
   ⚠️ **位图→CAD 线条**：用户贴图说"画进 CAD / 照着描 / 变成线"时，用内置工具 traceImage{image, draw:true}——**mode 不用猜，不传会自动按图片类型判定**（扁平/纯色插画/卡通/logo → flat 最省线最好看；线稿手绘照片 → arc）。想指定：flat / arc / centerline(好编辑) / outline(保笔画) / posterize(连背景一起搬)。要落地**直接 draw:true**（工具自己建图层），**禁止手工逐条 createPolyline**（烧轮次还不准）
