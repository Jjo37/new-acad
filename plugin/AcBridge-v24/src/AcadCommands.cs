...
public static class AcadCommands
{
  ...
  public static Task<object?> CreatePolylineAsync(JsonObject? parameters)
  {
    ...
    return CivilExecution.WriteAsync<object?>((doc, civilDoc, database, transaction) =>
    {
      ...
      // 2026-09-23: 支持逐项颜色（[r,g,b] 或 ACI 索引）
      var polyColor = ColorUtils.Parse(PluginRuntime.GetParameter(parameters, "color") as JsonNode);
      if (polyColor != null) polyline.Color = polyColor;
  ...
  public static Task<object?> GetEntityInfoAsync(JsonObject? parameters)
  {
    ...
    return CivilExecution.ReadAsync<object?>((doc, civilDoc, database, transaction) =>
    {
      ...
      // 直线
      // 填充（2026-09-23）
      else if (ent is Autodesk.AutoCAD.DatabaseServices.Hatch ht)
      {
        info["pattern"] = ht.PatternName;
        info["patternType"] = ht.PatternType.ToString();
        info["isSolid"] = string.Equals(ht.PatternName, "SOLID", StringComparison.OrdinalIgnoreCase) || ht.IsSolidFill;
        info["isGradient"] = ht.IsGradient;
        info["gradientName"] = ht.IsGradient ? ht.GradientName : null;
        info["gradientAngle"] = ht.IsGradient ? ht.GradientAngle : (double?)null;
        info["gradientOneColor"] = ht.IsGradient ? ht.GradientOneColorMode : (bool?)null;
        info["patternScale"] = ht.PatternScale;
        info["patternAngle"] = ht.PatternAngle;
        info["islandStyle"] = ht.HatchStyle.ToString();
        info["associative"] = ht.Associative;
        info["area"] = ht.Area;
        info["loopCount"] = ht.NumberOfLoops;
        var loopTypes = new List<string>();
        for (int li = 0; li < ht.NumberOfLoops; li++) loopTypes.Add(ht.LoopTypeAt(li).ToString());
        info["loops"] = loopTypes;
      }
  ...
  public static Task<object?> MeasureAreaAsync(JsonObject? parameters)
  {
    ...
    return CivilExecution.ReadAsync<object?>((doc, civilDoc, database, transaction) =>
    {
      ...
      // 2026-09-23: 填充（Hatch）也有面积 —— 此前 measureArea 对非曲线直接报错
      if (ent is Hatch hatchEnt)
      {
        return (object?)new Dictionary<string, object?>
        {
          ...
          ["type"] = typeName,
          ["area"] = hatchEnt.Area,
          ["pattern"] = hatchEnt.PatternName,
          ["isSolid"] = string.Equals(hatchEnt.PatternName, "SOLID", StringComparison.OrdinalIgnoreCase) || hatchEnt.IsSolidFill,
          ["loopCount"] = hatchEnt.NumberOfLoops,
