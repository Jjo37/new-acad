...
public static class AlignmentCommands
{
  ...
  public static Task<object?> SampleStationsAsync(JsonObject? parameters)
  {
    ...
    var stations = stationsNode.Select(node =>
      PluginRuntime.GetRequiredDoubleFromNode(node, "stations[]"))
      .ToArray();
  ...
  public static Task<object?> CreateAlignmentAsync(JsonObject? parameters)
  {
    var name = PluginRuntime.GetRequiredString(parameters, "name");
    // 支持两种模式：
    //  1) handle 模式：直接引用图中已有 polyline（保留弧段 bulge，不删原线）
    //  2) points 模式：传顶点坐标新建（可选 bulges）
    var existingHandle = PluginRuntime.GetOptionalString(parameters, "handle");
    var pointsNode = PluginRuntime.GetParameter(parameters, "points") as JsonArray;
    if (string.IsNullOrWhiteSpace(existingHandle) && (pointsNode == null || pointsNode.Count < 2))
    {
      throw new JsonRpcDispatchException("CIVIL3D.INVALID_INPUT", "createAlignment requires 'handle' (existing polyline) or at least two 'points'.");
    }
    ...
    return CivilExecution.WriteAsync<object?>((doc, civilDoc, database, transaction) =>
    {
      ...
      {
        // 引用已有 polyline（保留弧段）
        var existingId = database.GetObjectId(false, new Handle(Convert.ToInt64(existingHandle, 16)), 0);
        var existing = CivilObjectUtils.GetRequiredObject<Polyline>(transaction, existingId, OpenMode.ForRead);
      ...
      else
      {
        ...
        for (var index = 0; index < pointsNode!.Count; index++)
        {
          ...
          double bulge = 0;
          if (bulgesNode != null && index < bulgesNode.Count)
          {
            bulge = PluginRuntime.GetRequiredDoubleFromNode(bulgesNode[index], $"bulges[{index}]");
          }
          polyline.AddVertexAt(index, new Point2d(
            PluginRuntime.GetRequiredDoubleFromNode(point["x"], $"points[{index}].x"),
            PluginRuntime.GetRequiredDoubleFromNode(point["y"], $"points[{index}].y")), bulge, 0, 0);
        }
      ...
      {
        // 必须 false：true 会把 polyline 直线段当切线自动重算圆曲线，
        // 忽略原始 bulge 圆弧 → 带弧多段线转路线后圆弧丢失（2026-08-05 实测）
        AddCurvesBetweenTangents = false,
        EraseExistingEntities = string.IsNullOrWhiteSpace(existingHandle),
        PlineId = polylineId,
      };
