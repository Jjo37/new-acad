...
public static class CorridorEditingCommands
{
  ...
  public static Task<object?> AddCorridorSurfaceAsync(JsonObject? parameters)
  {
    ...
    // 2026-09-21: 是否等曲面真正生成完成再报完成（默认 true）。false = 旧"秒回"行为，用于 A/B 对比。
    var waitForBuild = PluginRuntime.GetOptionalBool(parameters, "waitForBuild") ?? true;
        ...
        await PluginRuntime.RunWithRequestContextAsync(
          ...
          async () =>
          {
            ...
            var result = await CivilExecution.WriteViaIdleAsync<object?>((doc, civilDoc, database, transaction) =>
            {
              ...
              // 2026-09-21: CorridorSurfaces.Add() 新建的曲面默认 IsBuild=true；显式置位固化语义。
              cs.IsBuild = true;
              try { PluginLog.Info("Corridor", "addCorridorSurface " + "step5: isBuild set"); } catch { }
              ...
              return new Dictionary<string, object?>
              {
                ...
                ["rebuilt"] = false,
                ["note"] = "曲面已挂上链接码并在本次提交时构建完成（App.Idle 路径，实测 ~0.1s）。用 getSurfaceStatistics 验证几何；如需强制重建可调 rebuildCorridor。",
              };
            });
              ...
              while (swWait.ElapsedMilliseconds < 300000)
              {
                ...
                try
                {
                  buildReady = await CivilExecution.ReadViaIdleAsync((doc, civilDoc, database, transaction) =>
                  {
                    ...
                    foreach (Autodesk.Civil.DatabaseServices.CorridorSurface c in corridor.CorridorSurfaces)
                    {
                      if (!string.Equals(c.Name, surfaceName, StringComparison.OrdinalIgnoreCase)) continue;
                      if (c.SurfaceId.IsNull) return false;
                      var surf = transaction.GetObject(c.SurfaceId, OpenMode.ForRead) as Autodesk.Civil.DatabaseServices.Surface;
                      if (surf is Autodesk.Civil.DatabaseServices.TinSurface tin)
                      {
                        return tin.GetTinProperties().NumberOfTriangles > 0;
                      }
                      return surf != null;
                    }
                    return false;
