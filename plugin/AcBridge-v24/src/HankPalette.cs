...
public static class HankPalette
{
    ...
    private static void BuildContent(UserControl c)
    {
        ...
        var btnRow = new TableLayoutPanel { Dock = DockStyle.Bottom, Height = 40, ColumnCount = 6, Padding = new Padding(6, 5, 6, 3), BackColor = Color.FromArgb(49, 50, 68) };
        // 2026-09-22: 5 列 → 6 列（新增「上传文件」）；仍横排一行，不碰竖排溢出那颗雷
        for (int ci = 0; ci < 6; ci++) btnRow.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100f / 6f));
        ...
        // 2026-09-22: 「上传文件」入口——不做粘贴图片，只把本地文件路径交给模型（readFile/readImage/traceImage 自行读取）
        var bFile = new Button { Text = Loc.T("上传文件", "Upload"), Dock = DockStyle.Fill, Margin = new Padding(2),
            BackColor = Color.FromArgb(49, 50, 68), ForeColor = Color.FromArgb(166, 227, 161),
            FlatStyle = FlatStyle.Flat, Font = new Font("Microsoft YaHei", 8, FontStyle.Bold) };
        bFile.Click += (_, _) => PickAndAttachFile();
        var fileTip = new ToolTip();
        fileTip.SetToolTip(bFile, Loc.T("选择本地文件，把路径填进输入框；补充说明后发送。模型会自己读取（图片可直接描成 CAD 线条）",
            "Pick a local file and put its path in the input box; add a note then Send. The model reads it (images can be traced into CAD)."));
        btnRow.Controls.Add(b, 0, 0); btnRow.Controls.Add(bFile, 1, 0); btnRow.Controls.Add(bCancel, 2, 0);
        btnRow.Controls.Add(bClear, 3, 0); btnRow.Controls.Add(bPlan, 4, 0); btnRow.Controls.Add(bMore, 5, 0);
    ...
    private static async System.Threading.Tasks.Task RestartRelayAsync()
    {
        ...
        try
        {
            // 2026-09-14 上架改造 D1：不再依赖计划任务（AcBridge-Relay）——插件直接拉起守护 launcher。
            // 优点：商店版没有计划任务也能自愈；任意安装路径都正确；端口/锁双重防重复（见 PluginRuntime）。
            var started = PluginRuntime.EnsureRelayRunning();
            SetStatus(started
                    ? Loc.T("● 守护进程已拉起", "● Daemon started")
                    : Loc.T("● 拉起未生效（详见 server\\relay-launcher.log）", "● Start had no effect (see server\\relay-launcher.log)"),
                started ? Color.FromArgb(166, 227, 161) : Color.FromArgb(249, 226, 175));
        }
    ...
    private static void ShowPortStatus()
    {
        ...
        catch (SysExc ex) { PluginLog.Debug("Palette", "端口状态诊断失败: " + ex.Message); }   // 2026-09-15: 不再静默吞异常
    }
    ...
    {
        // 2026-09-15: 统一走 PluginRuntime.ProjectRoot() 探测式解析。
        // 旧实现硬用 asmDir\..\..：dev 布局（DLL 在 <root>\plugin\AcBridge-v24\）恰好正确，
        // 但 Autoloader bundle（DLL 在 <bundle>\Contents\Windows\）差一级 → 根目录落到 bundle 本身，
        // tools\port-config.bat / config.json 全找不到 → 「端口 → 打开配置向导」静默无反应。
        try { return PluginRuntime.ProjectRoot(); } catch { return ""; }
    }

    // 2026-09-22: 上传文件入口——选定文件后把本地绝对路径插入输入框（模型用 readFile/readImage/traceImage 自行读取）
    private static void PickAndAttachFile()
    {
        try
        {
            using var dlg = new System.Windows.Forms.OpenFileDialog
            {
                Title = Loc.T("选择要交给 AI 的文件", "Choose a file for the AI"),
                Filter = "常用文件|*.png;*.jpg;*.jpeg;*.bmp;*.gif;*.tif;*.tiff;*.pdf;*.docx;*.doc;*.xlsx;*.xls;*.pptx;*.csv;*.txt;*.json;*.zip;*.dwg|图片|*.png;*.jpg;*.jpeg;*.bmp;*.gif;*.tif;*.tiff|所有文件|*.*",
                CheckFileExists = true,
                Multiselect = true,
            };
            var owner = input.FindForm();
            var picked = owner != null ? dlg.ShowDialog(owner) : dlg.ShowDialog();
            if (picked != System.Windows.Forms.DialogResult.OK || dlg.FileNames.Length == 0) return;
            ...
            var cur = input.Text ?? "";
            input.Text = string.IsNullOrWhiteSpace(cur)
                ? Loc.T("看一下这个文件：", "Take a look at this file: ") + paths
                : cur.TrimEnd() + Environment.NewLine + paths;
            input.SelectionStart = input.Text.Length;
            input.Focus();
            SetStatus(Loc.T("● 已附加文件路径（可补充说明后点发送）", "● File path attached (add a note, then Send)"), Color.FromArgb(166, 227, 161));
        }
        catch (SysExc ex) { SetStatus(Loc.T("● 选择文件失败: ", "● Pick file failed: ") + ex.Message, Color.FromArgb(243, 139, 168)); }
    }

    // 2026-09-22: 清理会话 / 全部清理 的二次确认——聊天记录现在真删（不可恢复），先报条数再问
    private static async Task<bool> ConfirmClearAsync(string scope)
    {
        int total = 0, current = 0;
        var curName = Loc.T("全局", "global");
        try
        {
            using var cts = new System.Threading.CancellationTokenSource(TimeSpan.FromSeconds(4));
            var resp = await hc.GetAsync(relayUrl + "/chat/scopes", cts.Token);
            var body = await resp.Content.ReadAsStringAsync(cts.Token);
            using var doc = System.Text.Json.JsonDocument.Parse(body);
            var root = doc.RootElement;
            if (root.TryGetProperty("current", out var cn) && cn.ValueKind == System.Text.Json.JsonValueKind.String) curName = cn.GetString() ?? curName;
            if (root.TryGetProperty("scopes", out var arr) && arr.ValueKind == System.Text.Json.JsonValueKind.Array)
            {
                foreach (var row in arr.EnumerateArray())
                {
                    var m = row.TryGetProperty("messages", out var mm) ? mm.GetInt32() : 0;
                    total += m;
                    var sname = row.TryGetProperty("scope", out var ss) ? (ss.GetString() ?? "") : "";
                    if (sname == curName) current = m;
                }
            }
        }
        catch { /* 统计失败不拦门，给通用提示 */ }
        ...
        if (scope == "all")
        {
            zh = "全部清理会真删：所有作用域的聊天记录（当前共 " + total + " 条）+ 任务计划 + 日志/缓存/MISSING 报告。\n\n不可恢复，确定继续？";
            en = "Clean everything permanently deletes all chat records (" + total + " messages), the task plan and logs/cache/reports.\n\nThis cannot be undone. Continue?";
        }
        else
        {
            zh = "清理会话会真删：当前作用域「" + curName + "」的聊天记录 " + current + " 条 + 会话上下文。\n\n不可恢复，确定继续？";
            en = "Clear chat permanently deletes the current scope \"" + curName + "\" chat records (" + current + " messages) and the session context.\n\nThis cannot be undone. Continue?";
        }
        return MessageBox.Show(Loc.T(zh, en), Loc.T("确认清理", "Confirm clean"),
    ...
    private static async Task ClearScope(string scope)
    {
        try
        {
            // 2026-09-22: 聊天记录真删（跟着按钮走）→ 先二次确认
            if ((scope == "chat" || scope == "all") && !await ConfirmClearAsync(scope)) return;
