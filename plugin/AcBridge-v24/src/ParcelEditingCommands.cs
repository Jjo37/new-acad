...
public static class ParcelEditingCommands
{
  ...
  {
    var siteName = PluginRuntime.GetRequiredString(parameters, "site");
    var parcelName = PluginRuntime.GetRequiredString(parameters, "parcel");
    return CivilExecution.WriteAsync<object?>((doc, civilDoc, database, transaction) =>
    {
      var site = CivilObjectUtils.GetRequiredObject<Site>(transaction, LookupUtils.GetSiteId(civilDoc, transaction, siteName), OpenMode.ForRead);
      var parcel = FindParcelByName(site, transaction, parcelName, OpenMode.ForWrite);
      parcel.Erase();
      return new Dictionary<string, object?> { ["deleted"] = true, ["site"] = siteName, ["parcel"] = parcelName };
    });
  }
