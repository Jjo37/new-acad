...
internal sealed class PortStatusForm : Form
{
    ...
    public PortStatusForm(int bridge, int relay, int mcp, string root)
    {
        ...
        var btnClose = new Button { Text = "关闭", Location = new Point(324, 128), Size = new Size(72, 30) };
        btnClose.Click += (_, _) => Close();
        var btnConfig = new Button { Text = "打开配置向导", Location = new Point(196, 128), Size = new Size(116, 30) };
        btnConfig.Click += (_, _) => OpenConfigWizard();
        var btnCopy = new Button { Text = "复制诊断", Location = new Point(108, 128), Size = new Size(80, 30) };
        btnCopy.Click += (_, _) => { try { Clipboard.SetText(BuildDiagnostic()); } catch { } };
        var btnRefresh = new Button { Text = "刷新", Location = new Point(24, 128), Size = new Size(72, 30) };
    ...
    {
        foreach (var r in _rows) r.Text = "  刷新中…";
        try
        {
            ...
            for (int i = 0; i < _rows.Length && i < list.Count; i++)
            {
                ...
                var owner = p.Listening ? $"  PID {p.OwnerPid} ({p.OwnerName})" : "  未启动";
                var note = p.Listening && !p.Expected ? "  ← 被其他程序占用！" : "";
        ...
        {
            for (int i = 0; i < _rows.Length; i++) _rows[i].Text = "  探测失败: " + ex.Message;
        }
    }
    ...
    private string BuildDiagnostic()
    {
        ...
        sb.AppendLine("new-acad 端口诊断 " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        var list = PortProbe.Probe(_bridge, _relay, _mcp);
        foreach (var p in list)
        {
            if (!p.Listening) sb.AppendLine($"  [--] {p.Port} {p.ServiceName}: 未启动");
            else if (p.Expected) sb.AppendLine($"  [OK] {p.Port} {p.ServiceName}: 监听中 PID {p.OwnerPid} ({p.OwnerName})");
            else sb.AppendLine($"  [!!] {p.Port} {p.ServiceName}: 被 {p.OwnerName} 占用 PID {p.OwnerPid}（非本服务进程）");
        }
        sb.AppendLine($"配置: bridgePort={_bridge} relayPort={_relay} mcpPort={_mcp}");
        return sb.ToString();
    }
    ...
    private void OpenConfigWizard()
    {
        try
        {
            ...
            if (File.Exists(cfg)) Process.Start(new ProcessStartInfo { FileName = "notepad.exe", Arguments = $"\"{cfg}\"" });
        }
        catch { }
    }
}
