...
public static class ProfileEditCommands
{
  ...
  public static Task<object?> ProfileViewCreateAsync(JsonObject? parameters)
  {
    ...
    // 2026-08-10: insertX/insertY 可选——不传时默认路线起点（比 (0,0) 合理，图框在用户可见处可拖动）；
    // AI 不需要猜坐标/问位置，创建后告知用户实际位置即可
    var insertX = PluginRuntime.GetOptionalDouble(parameters, "insertX");
    var insertY = PluginRuntime.GetOptionalDouble(parameters, "insertY");

    return CivilExecution.WriteAsync<object?>((doc, civilDoc, database, transaction) =>
    {
      var alignment = CivilObjectUtils.FindAlignmentByName(civilDoc, transaction, alignmentName);
      // 2026-08-10: 未指定位置 → 默认路线起点（用户可见、可拖动；AI 无需猜坐标）
      Point3d insertionPoint;
      if (insertX.HasValue && insertY.HasValue)
      {
        insertionPoint = new Point3d(insertX.Value, insertY.Value, 0);
      }
      else
      {
        // 2026-08-10: 25.0.58 Alignment 无 StartingPoint 属性 → 反射兼容 StartPoint/PointAtStart
        try
        {
          var sp = CivilObjectUtils.GetPropertyValue<Point3d?>(alignment, "StartPoint")
                ?? CivilObjectUtils.GetPropertyValue<Point3d?>(alignment, "PointAtStart");
          insertionPoint = sp.HasValue ? new Point3d(sp.Value.X, sp.Value.Y, 0) : new Point3d(0, 0, 0);
        }
        catch { insertionPoint = new Point3d(0, 0, 0); }
      }
      ...
      // ProfileView.Create 25.0.58 真实签名（api-inspect 反射确认，共 10 个重载）:
      //   简单版: Create(ObjectId alignmentId, Point3d insertPosition)
      //   带样式: Create(ObjectId alignmentId, Point3d insertPosition, string profileViewName, ObjectId profileViewBandSetId, ObjectId profileViewStyleId)
      //   带文档: Create(CivilDocument document, string profileViewName, ObjectId profileViewBandSetId, ObjectId alignmentId, Point3d insertPosition)
      // 注意：带 civilDoc 的重载在事务上下文内调用会抛 TargetInvocationException（2026-08-05 实测）→ 简单版优先，创建后补设名称/样式
      ...
      var pvId = (ObjectId?)(
        ...
        ?? CivilObjectUtils.InvokeStaticMethod(profileViewType, "Create",
          civilDoc, profileViewName, bandSetId, alignment.ObjectId, insertionPoint)
        ?? CivilObjectUtils.InvokeStaticMethod(profileViewType, "Create",
          civilDoc, profileViewName, ObjectId.Null, alignment.ObjectId, insertionPoint));
      ...
      // 简单版创建后补设名称/样式（默认名/样式可能不对）
      var profileView = CivilObjectUtils.GetRequiredObject<ProfileView>(
        transaction, pvId.Value, OpenMode.ForWrite);
      if (!string.IsNullOrEmpty(profileViewName))
      {
        try { profileView.Name = profileViewName; } catch { }
      }
      if (!styleId.IsNull)
      {
        try { profileView.StyleId = styleId; } catch { }
      }
      // 注意：不能手动 Commit —— WriteAsync 外层会自动提交，双重提交会崩 C3D（2026-08-05 实测）
