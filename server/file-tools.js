...
async function drawItems(items, layer, res, fillOpts) {
  ...
  res.drawn = { layer, ok: 0, fail: 0, samples: [], batched: false, mode: useFill ? 'hatch' : 'polyline' };
  ...
  res.note = '已画入当前图纸：图层 ' + layer + '，成功 ' + res.drawn.ok + ' / 失败 ' + res.drawn.fail +
    (useFill ? '（批量 importHatches 填充）' : res.drawn.batched ? '（批量 importVectorPaths）' : '（逐条 createPolyline）') + '。请提醒用户 ZOOM→E 查看。';
