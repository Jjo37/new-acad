...
function traceImage(args) {
  ...
  if (!args.mode) {
    try { const _im = engine.loadImage(p); kindInfo = engine.detectImageKind(_im); autoMode = kindInfo.kind === 'flat' ? 'flat' : 'arc'; }
    catch (_) { autoMode = 'arc'; }
  }
  const mode = String(args.mode || autoMode || 'arc').toLowerCase();
  if (!MODES.includes(mode)) throw new Error('mode 必须是 ' + MODES.join('|') + '（收到 ' + mode + '）');

  const layer = String(args.layer || ('TRACE_' + mode.toUpperCase()));
  const opts = {
    input: p, mode,
    simplify: num(args.simplify, 1.2), arcThreshold: num(args.arcThreshold, 0.02),
    minLength: num(args.minLength, 12), bridge: num(args.bridge, 6),
  ...
  const finish = (r) => {
    ...
    const outPath = resolveWritePath(path.join(getAgentWorkDir(), 'trace-' + base + '-' + mode + '.json'));
    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, JSON.stringify(r), 'utf8');
    const res = {
      ok: true, image: p, mode, layer, json: outPath, size: fs.statSync(outPath).size,
      stats: r.stats,
      autoMode: autoMode || undefined, imageKind: kindInfo || undefined,
    };
    if (args.draw === true || args.draw === 1) return drawItems(r.items, layer, res);
...
const FILE_TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: 'traceImage',
      description: '把位图（PNG/BMP/JPEG/GIF/TIFF）自动描摹成 CAD 矢量线条——用户贴图要求"画进 CAD/描成线条/照这个画"时用。mode 选择：flat=色块区域(★扁平/纯色插画：卡通、logo、矢量风壁纸——线最少效果最好) / arc=圆弧拟合(线稿、手绘、照片风格) / centerline=骨架细线(要能继续编辑) / outline=描边(保笔画粗细) / posterize=全色块(含背景)。不传 mode 会按图片类型自动判定（扁平→flat，其余→arc）。默认只生成路径 JSON；要画进图纸**直接带 draw:true**——工具会自己建图层、一次画完并回报成功/失败数（不要自己循环 createPolyline）。非 PNG/BMP 会自动走插件 exportImageGray 解码。',
      parameters: {
        type: 'object',
        properties: {
