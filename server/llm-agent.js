...
const { readFile, writeFile, deleteFile, listDir, readImage, FILE_TOOLS, getFileTools, getAgentWorkDir } = require('./file-tools.js');
...
📁 本地文件工具（readFile / listDir / writeFile / deleteFile / readImage + 插件 writeTextFile）：
...
async function handleMessage(cfg, history, text, snapshot, onProgress, signal, planNote) {
  ...
  while (turns < maxTurns) {
    ...
    // 2026-08-12 M6: 图片只注入一次——上轮注入的图片消息替换为文本占位(防 base64 每轮重发撑爆请求体)
    ...
    const resp = await callLLM(cfg, messages, cfg.timeoutMs || 120000, activeModel, signal);
      ...
      for (const tc of msg.tool_calls) {
        ...
        if (fn.name === 'readImage') {
          ...
          try {
            ...
            imageMsgIndex = messages.length; // 记录注入位置
            // 2026-08-10: 图片以 user 消息附加（OpenAI 兼容视觉格式 content 数组）——
            // 模型支持视觉即直接看图；不支持时 API 报错，由 AI 如实转述给用户（不预判不拦截）
            messages.push({
              role: 'user',
              content: [
    ...
    // 无工具调用 → 最终回复
    return { reply: msg.content || '', turns, calls };
  }
...
module.exports = { handleMessage, SYSTEM_PROMPT, callLLM, loadMemory, loadMemoryRelevant, saveMemory, loadKnowledge, loadKnowledgeIndex, MEMORY_FILE, KNOWLEDGE_FILE, clearActivePlan, hasActivePlan };
