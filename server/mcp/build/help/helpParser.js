...
export function listHelpTopicFiles(root) {
...
export function extractWrappedTopicHtml(jsText) {
...
export function parseHelpTopic(root, filePath, configuredVersion) {
    ...
    if (!title || plainBody.length < EMPTY_BODY_MINIMUM)
        return undefined;
...
export function toIndexEntry(topic) {
...
function extractImages($, rootElement, helpRoot, sourcePath, version) {
    ...
    rootElement.find("img[src]").each((_, node) => {
        ...
        if (!source || /^(?:https?:|data:)/i.test(source))
            return;
        if (element.hasClass("adsk-glyph-arrow") || element.closest(".uifinderbtn").length > 0)
            return;
        const assetPath = path.resolve(path.dirname(sourcePath), source);
        const fileName = path.basename(assetPath).toLowerCase();
        if (DECORATIVE_IMAGE_NAMES.has(fileName))
            return;
        let safeAssetPath;
        try {
        ...
        if (!fs.existsSync(safeAssetPath) || !fs.statSync(safeAssetPath).isFile())
            return;
...
function renderMarkdown($, rootElement, images, sourcePath) {
...
function renderNode($, node, lines, imageByPath, sourcePath) {
    const element = $(node);
    ...
    if (/^h[1-6]$/.test(tag)) {
        ...
        if (text)
            lines.push(`${"#".repeat(Math.min(Number(tag.slice(1)), 6))} ${text}`, "");
        return;
    }
    if (tag === "p") {
    ...
    if (tag === "table") {
        const rows = element.find("tr").toArray().map((row) => $(row).find("th, td").toArray().map((cell) => cleanText($(cell).text()))).filter((row) => row.some(Boolean));
...
function renderImage($, node, lines, imageByPath, sourcePath) {
...
function guidFromPath(filePath) {
...
function inferFeatureArea(text) {
...
function inferTopicType(raw, title, body) {
    const text = `${raw} ${title} ${body.slice(0, 600)}`.toLowerCase();
    if (text.includes("troubleshoot") || /^why\b/i.test(title))
        return "Troubleshooting";
    if (text.includes("dialog"))
        return "Dialog Reference";
    if (text.includes("command reference"))
        return "Command Reference";
    if (text.includes("tutorial"))
        return "Tutorial";
    if (raw.toLowerCase().includes("task") || /^to\s/i.test(title))
        return "Procedure";
    if (raw.toLowerCase().includes("reference"))
        return "Reference";
    return "Concept";
}
function inferTags(text, featureArea, topicType) {
    const lower = text.toLowerCase();
    const tags = new Set([
    ...
    for (const [name, terms] of objects) {
        if (terms.some((term) => lower.includes(term)))
            tags.add(`object:${name}`);
    }
    if (/\b(create|add|build|new)\b/.test(lower))
        tags.add("task:create");
    if (/\b(edit|modify|change)\b/.test(lower))
        tags.add("task:edit");
    if (/\b(troubleshoot|fix|not updating|wrong|missing)\b/.test(lower))
        tags.add("task:troubleshoot");
    return [...tags].sort();
}
