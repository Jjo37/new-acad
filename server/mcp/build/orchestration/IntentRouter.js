...
function extractNamedValue(request, fieldLabel) {
    const expression = new RegExp(`${fieldLabel}\\s*[:=]\\s*(.+?)(?=\\s+[a-z][a-z0-9_]*(?:\\s+[a-z][a-z0-9_]*)?\\s*[:=]|[,;]|$)`, "i");
    const match = request.match(expression);
    return match?.[1]?.trim();
}
function extractNamedNumber(request, fieldLabel) {
    const expression = new RegExp(`${fieldLabel}\\s*[:=]?\\s*(-?\\d+(?:\\.\\d+)?)`, "i");
    const match = request.match(expression);
    return match ? Number(match[1]) : undefined;
}
function extractPathValue(request, fieldLabel) {
...
function extractPoint3D(request, fieldLabel) {
    const namedMatch = new RegExp(`${fieldLabel}\\s*[:=]\\s*\\(?\\s*(-?\\d+(?:\\.\\d+)?)\\s*,\\s*(-?\\d+(?:\\.\\d+)?)(?:\\s*,\\s*(-?\\d+(?:\\.\\d+)?))?\\s*\\)?`, "i").exec(request);
    if (!namedMatch) {
        return undefined;
    }
    return {
...
function extractObjectName(request, objectType) {
...
function extractSimpleNamedObject(request, objectType) {
    return extractNamedValue(request, `${objectType}(?:\\s*name)?`)
        ?? extractObjectName(request, objectType);
}
function extractSurfacePair(request) {
...
function scoreRequest(normalizedRequest, entry) {
    let score = 0;
...
export function routeIntent(request) {
    const normalizedRequest = normalize(request);
    ...
    if (exactToolMatch) {
        const missingFields = exactToolMatch.requiredFields.filter((field) => {
            const value = extractedParams[field];
