...
function resolveApprovalTarget(args) {
    ...
    if (resolved.action !== args.action) {
        throw new Error(`Parameters do not resolve to action '${args.action}' for tool '${args.toolName}'. ` +
...
export function registerApprovalTool(server) {
    const previewHandler = async (rawArgs) => {
        try {
            const args = rawArgs;
    ...
    const requestHandler = async (rawArgs) => {
        try {
            const args = rawArgs;
            const target = resolveApprovalTarget(args);
            const receipt = await approvalPolicy.requestApproval({
                ...
                safeForRetry: target.actionDefinition.safeForRetry,
                requiresActiveDrawing: target.actionDefinition.requiresActiveDrawing,
            }, args.parameters, (args.ttlSeconds ?? 300) * 1000);
            return successResult({
                status: "approved",
                toolName: args.toolName,
                action: args.action,
    ...
    server.registerTool("civil3d_preview_action", {
    ...
    server.registerTool("civil3d_request_approval", {
