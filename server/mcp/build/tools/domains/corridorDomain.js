...
// 廊道创建 schema (2026-08-16 道路建模能力补齐，2026-09-14 从 build 回移补回 src)
...
// ─── Domain definition ────────────────────────────────────────────────────────
export const CORRIDOR_DOMAIN_DEFINITION = {
    domain: "corridor",
    actions: {
        list: {
            action: "list",
            ...
            execute: async () => await withApplicationConnection(async (appClient) => await appClient.sendCommand("listCorridors", {})),
        },
        get: {
            action: "get",
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("getCorridor", {
                name: args.name,
            })),
        },
        rebuild: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("rebuildCorridor", {
                name: args.name,
            })),
        },
        get_surfaces: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("getCorridorSurfaces", {
                name: args.name,
            })),
        },
        get_feature_lines: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("getCorridorFeatureLines", {
                name: args.name,
            })),
        },
        compute_volumes: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("computeCorridorVolumes", {
                name: args.name,
        ...
        summary: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => {
                const corridor = CorridorDetailResponseSchema.parse(await appClient.sendCommand("getCorridor", { name: args.name }));
                const surfaceInventory = CorridorSurfacesResponseSchema.parse(await appClient.sendCommand("getCorridorSurfaces", { name: args.name }));
                const selectedCorridorSurface = args.corridorSurface
                    ?? (surfaceInventory.surfaces.length === 1 ? surfaceInventory.surfaces[0].name : null);
                const referenceSurface = args.referenceSurface ?? null;
                let volumeAnalysis = null;
                if (selectedCorridorSurface && referenceSurface) {
                    volumeAnalysis = CorridorVolumesResponseSchema.parse(await appClient.sendCommand("computeCorridorVolumes", {
                        name: args.name,
                        corridorSurface: selectedCorridorSurface,
                        referenceSurface,
                    }));
                }
                const regionCount = corridor.baselines.reduce((total, baseline) => total + baseline.regions.length, 0);
                const totalRegionLength = corridor.baselines.reduce((baselineTotal, baseline) => baselineTotal + baseline.regions.reduce((regionTotal, region) => regionTotal + (region.endStation - region.startStation), 0), 0);
        ...
        target_mapping_get: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("getCorridorTargetMappings", {
        ...
        target_mapping_set: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("setCorridorTargetMappings", {
                corridorName: args.name,
        ...
        region_add: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("addCorridorRegion", {
                corridorName: args.name,
                baselineIndex: args.baselineIndex ?? 0,
        ...
        create: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("createCorridor", {
                corridorName: args.corridorName,
                alignmentName: args.alignmentName,
                assemblyName: args.assemblyName,
                baselineName: args.baselineName ?? "Baseline-1",
                regionName: args.regionName ?? "Region-1",
        ...
        region_delete: {
            ...
            execute: async (args) => await withApplicationConnection(async (appClient) => await appClient.sendCommand("deleteCorridorRegion", {
                corridorName: args.name,
                baselineIndex: args.baselineIndex ?? 0,
    ...
    exposures: [
        {
            ...
            description: "廊道聚合工具：查询/读取/重建/目标映射/加删区域/体积",
        ...
        },
        {
            ...
            description: "廊道加区域 {corridorName, assemblyName, startStation, endStation}",
            inputShape: {
                corridorName: z.string(),
                baselineIndex: z.number().int().nonnegative().optional(),
                assemblyName: z.string(),
        ...
        },
        {
            ...
            description: "廊道删区域 {corridorName, regionIndex}",
            inputShape: {
                corridorName: z.string(),
                baselineIndex: z.number().int().nonnegative().optional(),
                regionIndex: z.number().int().nonnegative(),
        ...
        {
            toolName: "civil3d_corridor_create",
            displayName: "Civil 3D Corridor Create",
            description: "创建廊道 {corridorName, alignmentName, assemblyName, baselineName?, regionName?, startStation?, endStation?, profileName?, surfaceName?, style?}——路线+装配→廊道",
            inputShape: {
                corridorName: z.string(),
                alignmentName: z.string(),
                assemblyName: z.string(),
            ...
            resolveAction: (rawArgs) => ({
                ...
                args: {
                    ...
                    assemblyName: rawArgs.assemblyName,
                    baselineName: rawArgs.baselineName,
