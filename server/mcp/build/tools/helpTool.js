...
let defaultManager;
export function getCivil3DHelpManager() {
    defaultManager ??= new Civil3DHelpManager();
    return defaultManager;
}
export function registerHelpTool(server, manager = getCivil3DHelpManager(), videoCatalog = getAutodeskVideoCatalog()) {
    const handler = async (input, extra) => {
        try {
            const progress = createProgressReporter(extra);
            await progress(0, 100, `Civil 3D help: starting '${input.action}'.`);
            if (input.action === "search_videos") {
                if (!input.query?.trim())
                    throw invalidInput("The search_videos action requires a non-empty query.");
            ...
            if (input.action === "search") {
                if (!input.query?.trim())
                    throw invalidInput("The search action requires a non-empty query.");
            ...
            if (input.action === "get_topic") {
                ...
                if (!idOrUri)
                    throw invalidInput("The get_topic action requires id or uri.");
                const version = input.version ?? versionFromTopicUri(input.uri);
                const topic = await manager.getTopic(version, idOrUri);
                if (!topic)
                    return toolError("CIVIL3D.OBJECT_NOT_FOUND", "Civil 3D help topic was not found.", -32004);
                const videos = input.includeVideos === false
                    ? []
                ...
                if (input.includeImages !== false) {
                    ...
                    for (const image of selectedImages) {
                        if (totalBytes + image.size > maxTotalBytes)
                            continue;
                        content.push({
                            type: "image",
            ...
            if (process.env.CIVIL3D_ENABLE_HELP_REINDEX !== "true") {
                return toolError("CIVIL3D.CONFLICT", "Civil 3D help reindex is disabled. Set CIVIL3D_ENABLE_HELP_REINDEX=true to enable it.", -32009);
            }
    ...
    captureToolHandler("civil3d_help", (args, extra) => handler(args, extra));
    server.registerTool("civil3d_help", {
...
export function registerHelpResources(server, manager = getCivil3DHelpManager(), videoCatalog = getAutodeskVideoCatalog()) {
    server.registerResource("civil3d-help-topic", new ResourceTemplate("civil3d://help/topics/{version}/{topicId}", { list: undefined }), {
        ...
        if (!topic)
            throw new Error("Civil 3D help topic was not found.");
        return {
            contents: [{ uri: uri.href, mimeType: "text/markdown", text: topic.markdown }],
        };
    });
    server.registerResource("civil3d-help-image", new ResourceTemplate("civil3d://help/images/{version}/{imageId}", { list: undefined }), {
        ...
        if (!image)
            throw new Error("Civil 3D help image was not found.");
        return {
            contents: [{
                    uri: uri.href,
                    mimeType: image.mimeType,
    ...
    server.registerResource("civil3d-help-video", new ResourceTemplate("civil3d://help/videos/{version}/{videoId}", { list: undefined }), {
...
function jsonResult(action, result, additionalContent = []) {
    return {
        content: [
            { type: "text", text: JSON.stringify(result, null, 2) },
            ...additionalContent,
...
function videoContentBlocks(videos) {
...
function toolError(code, message, rpcCode) {
    return {
...
function errorCode(error) {
    if (error && typeof error === "object" && "code" in error && typeof error.code === "string")
        return error.code;
    if (/not found|not installed/i.test(error instanceof Error ? error.message : String(error)))
...
function safeIndexStatus(status) {
