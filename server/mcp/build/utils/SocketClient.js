...
export class Civil3DRpcError extends Error {
    code;
    rpcCode;
    constructor(message, code, rpcCode) {
        super(message);
        this.code = code;
...
export class ApplicationClientConnection {
    ...
    cancel(error = new Civil3DRpcError("Civil 3D request was cancelled by the caller.", "CIVIL3D.CANCELLED", -32010)) {
        this.rejectPendingRequests(error);
        this.socket.destroy();
        this.isConnected = false;
    }
                    ...
                    try {
                        ...
                        if (response.error) {
                            ...
                            const rpcCode = typeof response.error.code === "number"
                                ? response.error.code
                                : -32000;
                            reject(new Civil3DRpcError(response.error.message || "Unknown error from Civil 3D plugin", domainCode, rpcCode));
                        }
                        else {
