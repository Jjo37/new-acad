...
export const DRAWING_RUNTIME_DOMAIN_DEFINITION: DomainToolDefinition = {
  ...
  exposures: [
    { toolName: "civil3d_drawing", displayName: "Civil 3D Drawing", description: "图纸聚合工具：读取图纸信息/设置/选择集/新建保存", inputShape: { action: z.enum(["info", "new", "save", "undo", "redo", "settings", "selected_objects_info", "list_object_types"]), templatePath: z.string().optional(), saveAs: z.string().optional(), overwrite: z.boolean().optional(), steps: z.number().int().min(1).max(10).optional(), limit: z.number().optional() }, supportedActions: ["info", "new", "save", "undo", "redo", "settings", "selected_objects_info", "list_object_types"], resolveAction: (rawArgs) => ({ action: String(rawArgs.action ?? ""), args: rawArgs }) },
    { toolName: "civil3d_get_drawing_info", displayName: "Get Drawing Info", description: "读取当前图纸信息（文件名、单位、对象计数）", inputShape: {}, supportedActions: ["info"], resolveAction: () => ({ action: "info", args: { action: "info" } }) },
    { toolName: "civil3d_get_selected_civil_objects_info", displayName: "Get Selected Civil Objects Info", description: "读取选中 Civil 3D 对象的详细信息", inputShape: { limit: z.number().optional() }, supportedActions: ["selected_objects_info"], resolveAction: (rawArgs) => ({ action: "selected_objects_info", args: { action: "selected_objects_info", ...rawArgs } }) },
    { toolName: "civil3d_list_civil_object_types", displayName: "List Civil Object Types", description: "列出图纸中的 Civil 3D 对象类型及数量", inputShape: {}, supportedActions: ["list_object_types"], resolveAction: () => ({ action: "list_object_types", args: { action: "list_object_types" } }) },
  ],
};
