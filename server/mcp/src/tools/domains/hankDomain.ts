...
const getSelectionExecute = async () =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("getSelection", {}));
const getSelectionShape = {};

// ========== 2. getEntityInfo ==========
const GetEntityInfoArgs = z.object({ handle: z.string().min(1) });
const getEntityInfoExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("getEntityInfo", { handle: args.handle }));
...
const setEntityColorExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("setEntityColor", { handle: args.handle, color: args.color }));
...
const mirrorEntityExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("mirrorEntity", { handle: args.handle }));
...
const executeCommandExecute = async (args: { command?: unknown }) => {
  ...
  return await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("executeCommand", { command: cmdName }));
};
const executeCommandShape = { command: z.string().min(1) };

const DeleteEntityArgs = z.object({ handle: z.string().min(1) });
const deleteEntityExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("deleteEntity", { handle: args.handle }));
...
const moveEntityExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("moveEntity", { handle: args.handle, dx: args.dx, dy: args.dy }));
...
const copyEntityExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("copyEntity", { handle: args.handle, dx: args.dx, dy: args.dy }));
...
const rotateEntityExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("rotateEntity", { handle: args.handle, angle: args.angle, cx: args.cx, cy: args.cy }));
...
const scaleEntityExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("scaleEntity", { handle: args.handle, factor: args.factor, cx: args.cx, cy: args.cy }));
...
const selectByCriteriaExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("selectByCriteria", { criteria: args.criteria, value: args.value }));
const selectByCriteriaShape = { criteria: z.enum(["layer", "type", "color"]), value: z.string().min(1) };

const GetLayersArgs = z.object({});
const getLayersExecute = async () =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("getLayers", {}));
const getLayersShape = {};

const CreateLayerArgs = z.object({ name: z.string().min(1), color: z.number().int().min(1).max(255).optional() });
const createLayerExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("createLayer", { name: args.name, color: args.color }));
...
const createCircleExecute = async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand("createCircle", { center: args.center, diameter: args.diameter }));
...
const genExec = (method) => async (args) =>
  await withApplicationConnection(async (appClient) =>
    await appClient.sendCommand(method, args || {}));
...
const layExec = (method) => async (args) =>
  await withApplicationConnection(async (appClient) => appClient.sendCommand(method, { layerName: args.layerName }));
const layShape = { layerName: z.string().min(1) };

const LayerIsolateArgs = z.object({ handles: z.array(z.string()).optional() });
const layerIsolateExec = async (args) =>
  await withApplicationConnection(async (appClient) => appClient.sendCommand("layerIsolate", { handles: args.handles }));
...
const layerMatchExec = async (args) =>
  await withApplicationConnection(async (appClient) => appClient.sendCommand("layerMatch", { sourceHandle: args.sourceHandle, targetHandle: args.targetHandle }));
