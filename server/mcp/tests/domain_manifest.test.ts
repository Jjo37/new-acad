...
describe("domain manifest migration", () => {
  ...
  it("generates canonical point domain with point-group and export aliases", () => {
    ...
    const createCogoPoint = GENERATED_TOOL_CATALOG_ENTRIES.find((e) => e.toolName === "civil3d_create_cogo_point");
  ...
  it("generates plugin and docs helper domains including orchestrate", () => {
    ...
    const capabilities = GENERATED_TOOL_CATALOG_ENTRIES.find((e) => e.toolName === "civil3d_list_tool_capabilities");
  ...
  it("merges generated entries back into the main tool catalog", () => {
    ...
    expect(alignment).toBeDefined();
    expect(alignment!.description).toContain("路线聚合");
    expect(surface).toBeDefined();
