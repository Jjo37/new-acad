...
- **一键给廊道加曲面（算量用）**（2026-09-11 新增）：`addCorridorSurface{name, surfaceName?, codes?[], asBreakLine?}`——把廊道**链接码**挂成曲面数据并**重建**；**走 job 通道**（返回 `{jobId, state:"running"}` 属正常，用 `getJobStatus{jobId}` 轮询，重建耗时 >25s 正常）。不传 `codes` 会**自动收集**廊道链接码；result 里 `surfaceId` = 曲面对象 handle，另有 `addedLinkCodes / isBuild / surfaceCount`。**要出曲面/算量必须调用它**（createCorridor 建的廊道默认无曲面数据，surfaceCount=0）。前提：自定义部件的**链接（links）**要有码——形状码只进材料表、不进曲面。
