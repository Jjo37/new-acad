...
const llmHistory = [];                                     // 兼容占位（旧引用已改按作用域分桶）
// 2026-09-22: llm 模式会话历史按作用域分桶并落盘（''=全局 / 项目名）——切项目不串味、重启可续
const llmHistories = new Map();
function histFile(scope) { return path.join(chatScopeDir(scope), 'llm-history.json'); }
function getHistory(scope) {
  const key = scope || '';
  if (llmHistories.has(key)) return llmHistories.get(key);
  let arr = [];
  try { const f = histFile(key); if (fs.existsSync(f)) { const j = JSON.parse(fs.readFileSync(f, 'utf8')); if (Array.isArray(j)) arr = j; } } catch (_) {}
  llmHistories.set(key, arr);
  return arr;
}
function saveHistory(scope) {
  const key = scope || '';
  try {
    const slim = (llmHistories.get(key) || []).filter(m => m && (m.role === 'user' || m.role === 'assistant'))
      .slice(-60).map(m => ({ role: m.role, content: typeof m.content === 'string' ? m.content.slice(0, 8000) : JSON.stringify(m.content).slice(0, 8000) }));
    const f = histFile(key);
    fs.mkdirSync(path.dirname(f), { recursive: true });
    fs.writeFileSync(f, JSON.stringify(slim, null, 1), 'utf8');
  } catch (e) { log('CHAT', 'history save failed: ' + e.message); }
}
...
// ---------- 面板聊天落盘 v2（2026-09-22 分作用域）----------
// 全局: exchange/panel-chat-YYYY-MM-DD.jsonl ；项目: exchange/projects/<项目>/panel-chat-YYYY-MM-DD.jsonl
// 作用域 = 消息发出时所在项目（''=全局）。保留策略跟随按钮：/clear chat 真删当前作用域，/clear all 连所有项目一起删。
const CHAT_DIR = path.join(__dirname, '..', 'exchange');
function chatScopeDir(scope) { return scope ? path.join(CHAT_DIR, 'projects', scope) : CHAT_DIR; }
function chatLogPath(day, scope) {
  const d = day ? new Date(day + 'T00:00:00') : new Date();
  const p2 = (x) => String(x).padStart(2, '0');
  return path.join(chatScopeDir(scope), 'panel-chat-' + d.getFullYear() + '-' + p2(d.getMonth() + 1) + '-' + p2(d.getDate()) + '.jsonl');
}
function chatAppend(rec) {
  try {
    const sc = (rec && rec.scope) || '';
    fs.mkdirSync(chatScopeDir(sc), { recursive: true });
    fs.appendFileSync(chatLogPath(null, sc), JSON.stringify(Object.assign({ time: fmtLocal() }, rec)) + '\n');
  } catch (e) { log('CHAT', 'append failed: ' + e.message); }
}
function chatRead(day, limit, scope) {
  const fp = chatLogPath(day, scope);
  if (!fs.existsSync(fp)) return [];
  const lines = fs.readFileSync(fp, 'utf8').split('\n').filter(Boolean);
  const out = [];
  for (const ln of lines) { try { out.push(JSON.parse(ln)); } catch (_) {} }
  return limit > 0 ? out.slice(-limit) : out;
}
// 各作用域记录盘点（全局 + 各项目）
function chatListScopes() {
  const rows = [];
  const scan = (scope, dir) => {
    let files = [], messages = 0, last = '';
    try {
      if (fs.existsSync(dir)) {
        files = fs.readdirSync(dir).filter(f => /^panel-chat-.*\.jsonl$/.test(f)).sort();
        for (const f of files) {
          const lines = fs.readFileSync(path.join(dir, f), 'utf8').split('\n').filter(Boolean);
          messages += lines.length;
          if (lines.length) { try { const t = JSON.parse(lines[lines.length - 1]).time || ''; if (t > last) last = t; } catch (_) {} }
        }
      }
    } catch (_) {}
    if (files.length || scope === '') rows.push({ scope: scope || '(全局)', dir, files: files.length, messages, last });
  };
  scan('', CHAT_DIR);
  try {
    const pdir = path.join(CHAT_DIR, 'projects');
    if (fs.existsSync(pdir)) for (const d of fs.readdirSync(pdir)) {
      const dd = path.join(pdir, d);
      if (fs.statSync(dd).isDirectory()) scan(d, dd);
    }
  } catch (_) {}
  return rows;
}
// 真删记录（含 llm-history.json）：scope='' 全局 / 项目名 / '*' 全部
function chatClear(scope) {
  const targets = [];
  if (scope === '*') {
    targets.push(CHAT_DIR);
    try {
      const pdir = path.join(CHAT_DIR, 'projects');
      if (fs.existsSync(pdir)) for (const d of fs.readdirSync(pdir)) {
        const dd = path.join(pdir, d);
        if (fs.statSync(dd).isDirectory()) targets.push(dd);
      }
    } catch (_) {}
  } else targets.push(chatScopeDir(scope));
  let removed = 0;
  for (const dir of targets) {
    try {
      if (!fs.existsSync(dir)) continue;
      for (const f of fs.readdirSync(dir)) {
        if (/^panel-chat-.*\.jsonl$/.test(f) || f === 'llm-history.json') {
...
// 2026-09-14: 往 /replies/stream 的客户端推任意事件（面板订这条流；progress 以前只发通用流，面板收不到）
function sseBroadcastReplyEvent(event, data) {
  const msg = 'event: ' + event + '\ndata: ' + JSON.stringify(data) + '\n\n';
...
function pushReply(text, messageId) {
  ...
  // 2026-09-22: 回复归到该消息发出时所在作用域（中途切项目也不串）
  const _scopeEntry = messageId ? inbox.find(x => x.id === messageId) : null;
  const _scope = _scopeEntry ? (_scopeEntry.scope || '') : (currentProject ? sanitizeProjectName(currentProject) : '');
  chatAppend({
    role: /^\u26a0/.test(String(text)) ? 'system' : 'assistant',
    text: text,
    id: messageId || null,
    channel: 'panel',
    mode: RELAY_MODE === 'llm' ? 'llm' : 'cad',
    drawing: (selectionCache && selectionCache.docName) || null,
    scope: _scope,
    project: _scope || null,
  });   // 2026-09-22 聊天落盘（唯一回复出口）
  log('REPLY', text.slice(0, 300));
}
...
function handleSessionMessage(sk, msg) {
  ...
  if (msg.role === 'assistant' && msg.content) {
    ...
    if (text && !text.includes('NO_REPLY')) {
      ...
      if (text) {
        pushReply(text);
        log('GW_AUTO', '[CAD] ' + text.slice(0, 300));
      }
    }
  }
}
...
2. 调用 C3D 插件：HTTP POST http://127.0.0.1:19876/tcp，body 为 JSON-RPC：{"jsonrpc":"2.0","id":1,"method":"方法名","params":{...}}
   ...
   ⚠️ **不确定方法名/参数时，先调 listMethods 拿实时方法清单（反射自动扫描，数百个），再选方法。禁止猜方法名**
   ⚠️ **有依赖关系的操作必须串行**：等前一步返回成功后再发下一步（如 建路线→设桩号→建纵断面 是依赖链，禁止并发发；并发会排队卡死 25s 超时）
   ⚠️ **位图→CAD 线条**：用户贴图说"画进 CAD / 照着描 / 变成线"时，用内置工具 traceImage{image, draw:true}——**mode 不用猜，不传会自动按图片类型判定**（扁平/纯色插画/卡通/logo → flat 最省线最好看；白底线稿/扫描件（含实心墨块）→ outline 保外轮廓；其余照片/彩图 → arc）。想指定：flat / arc / centerline(好编辑) / outline(保笔画) / posterize(连背景一起搬)。掩膜来源可传 mask 参数（默认 auto：白底线稿/扫描件自动走墨迹阈值，照片/彩图走边缘检测）。要落地**直接 draw:true**（工具自己建图层），**禁止手工逐条 createPolyline**（烧轮次还不准）
   🖼️ **图片→CAD 分诊（别跳）**：①图上**有尺寸标注** → 别描像素，走标注驱动重建：先列尺寸表 → 尺寸链闭合自检（分段和=总长）→ 不确定项问用户 → 按真实尺寸画（锚点+相对尺寸）→ 回读对账；②完全没尺寸 → **必问一次**"图上有没有哪一段长度你知道？哪怕一个"，给三个默认选项（按图比例/给一个参考尺寸做单点标定/给目标总尺寸），**绝不硬猜尺寸，只问这一次**；③不是工程图（照片/实物/插画）→ 照形描或按目标总尺寸缩放，并如实说这是示意图。**铁律：坐标只来自标注或用户给的尺寸，像素几何只用于识别形状与拓扑**。详见 knowledge/image-to-cad.md（项目根目录或安装目录下）
   ⚠️ **禁止声称"插件不支持/没有/不能"**：这类结论**必须先用 listMethods 查到确证**；查不到就说【MISSING】或"需要新方法补充"，**禁止自己编造插件限制**（曾把明明可用的 deleteLayer / getEntitiesByLayer 说成不可用，把清理甩给用户）
   🧹 **描摹或批量绘制后自清理**：重描、换模式、换参数前，先删掉自己上一版：getEntitiesByLayer{layer} → deleteEntities{handles} → deleteLayer{name}（三者均可用、支持翻页取全量）。**不许让用户手动 LAYDEL**；最终只保留选定方案的那一层
3. 常用 API（全部可用，直接调）：
   ...
   - 填充: createHatch{points|loops,pattern,scale,angle,color,layer} / importHatches{items} / editHatch{handle,pattern,scale,angle,color,layer,origin,loops}——图案(SOLID/ANSI31...)、比例、角度、颜色、图层都能改，也能传 loops 整体换边界；填充可被 type=HATCH 选中、measureArea 量面积、getEntityInfo 读属性
4. 不需要汇报过程，直接执行
5. 不需要说"正在操作..."之类的话
6. 别带 [PANEL] 标记
...
async function processLlmMessage(entry, msg, lcfg) {
  ...
  const llmScope = (entry && entry.scope) || '';   // 2026-09-22: 该消息的作用域（发出时所在项目）
  ...
  try {
    const onProgress = (p) => {
      if (p && p.text) {
        ...
        try { sseBroadcastReplyEvent('progress', { text: progressCache.text }); } catch (_) {}   // 2026-09-14: 面板实时进度
        log('PROG', progressCache.text);
      }
    };
    const histNow = getHistory(llmScope);   // 2026-09-22: 按作用域取历史
    const r = await handleMessage(lcfg, histNow, msg, selSnapshot, onProgress, abortCtrl.signal, planNote, (m) => log('LLM', m));   // 2026-09-15: 代理侧诊断落 relay.log
    // 2026-09-14: 任务结束 → 清进度（面板状态栏立即回落到"在线"，不等 10s 轮询）
    progressCache = null;
    try { sseBroadcastReplyEvent('progress', { text: '' }); } catch (_) {}
    histNow.push({ role: 'user', content: msg });
    if (r.reply) histNow.push({ role: 'assistant', content: r.reply });
    if (histNow.length > 24) histNow.splice(1, histNow.length - 24);
    saveHistory(llmScope);   // 2026-09-22: 落盘（重启可续）
...
const server = http.createServer((req, res) => {
  ...
  // ---- GET /chat/scopes — 各作用域记录盘点（全局 + 各项目） ----
  if (req.method === 'GET' && req.url.split('?')[0] === '/chat/scopes') {
    try { res.end(JSON.stringify({ ok: true, current: currentProject ? sanitizeProjectName(currentProject) : '(全局)', scopes: chatListScopes() })); }
    catch (e) { res.end(JSON.stringify({ ok: false, error: e.message })); }
    return;
  }

  // ---- GET /chat — 面板聊天记录回读（?scope=global|项目名&date=YYYY-MM-DD&limit=N；默认当前作用域） ----
  if (req.method === 'GET' && req.url.split('?')[0] === '/chat') {
    try {
      const u = new URL(req.url, 'http://localhost');
      const day = u.searchParams.get('date') || '';
      const limit = parseInt(u.searchParams.get('limit') || '0', 10) || 0;
      const qs = u.searchParams.get('scope');
      const scope = qs === null ? (currentProject ? sanitizeProjectName(currentProject) : '') : (qs === 'global' ? '' : qs);
      const msgs = chatRead(day, limit, scope);
      const fp = chatLogPath(day, scope);
      res.end(JSON.stringify({ ok: true, scope: scope || '(全局)', file: fp, date: path.basename(fp).replace('panel-chat-', '').replace('.jsonl', ''), count: msgs.length, messages: msgs }));
    } catch (e) { res.end(JSON.stringify({ ok: false, error: e.message })); }
    return;
  }
  ...
  if (req.method === 'POST' && req.url === '/send') {
    ...
    req.on('end', async () => {
      try {
        ...
        const entry = { id: (++msgIdCounter).toString(36), text: msg, time: new Date().toISOString(), status: 'pending', sent: false, scope: currentProject ? sanitizeProjectName(currentProject) : '' };   // 2026-09-22: 记录发出时作用域
        inbox.push(entry);
        saveInbox();
        sseBroadcast('message', entry);
        chatAppend({ role: 'user', text: msg, id: entry.id, channel: 'panel', mode: RELAY_MODE === 'llm' ? 'llm' : 'cad', scope: entry.scope, project: entry.scope || null });   // 2026-09-22 聊天落盘（作用域=发出时项目）
    ...
    req.on('end', () => {
      try {
        ...
        if (scopeSet.has('chat')) {
          ...
          // 2026-09-22: 真删聊天记录（跟着按钮走。scope=all 时连所有项目一起删）
          const isAllScope = scopeSet.has('plan');
          const delScope = isAllScope ? '*' : (currentProject ? sanitizeProjectName(currentProject) : '');
          if (isAllScope) llmHistories.clear(); else llmHistories.delete(delScope);
          result['chatRecordsDeleted'] = chatClear(delScope);
          result.chat = true;
        }
  ...
  if (req.method === 'GET' && req.url === '/replies/stream') {
    ...
    // 2026-09-14: 回放当前进度（面板重连/重开面板后立刻同步，不用等下一次 progress）
    if (progressCache) {
      try { res.write('event: progress\ndata: ' + JSON.stringify({ text: progressCache.text, time: new Date().toISOString() }) + '\n\n'); } catch (_) {}
    }
