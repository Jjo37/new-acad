...
if (Test-Path "$ROOT\server\relay.log") {
  ...
  try {
    ...
    Write-Host "[跳过] relay.log 被占用（relay 正在运行），不影响使用" -ForegroundColor Yellow
  }
}
...
if (Test-Path "$ROOT\server\relay-launcher.log") {
  ...
  try {
    ...
    Write-Host "[跳过] relay-launcher.log 被占用（launcher 正在运行），不影响使用" -ForegroundColor Yellow
  }
}
...
Write-Host ("释放空间: {0:N1} KB" -f ($freed / 1KB))
if (-not $All) {
  Write-Host "提示: 报告文件在 exchange\export，确认不再需要后加 -All 一起清理" -ForegroundColor Yellow
}
