...
// 2026-09-14 v1.4.2: D1 把「schtasks 兜底」换成「插件自启 relay」→ 断言同步更新
const d1Mark = dll.includes(Buffer.from('relay launcher started', 'utf16le'))
  && dll.includes(Buffer.from('relay-launcher.lock', 'utf16le'))
  && dll.includes(Buffer.from('relay already listening on :', 'utf16le'));
chk('DLL 含 relay 自启（D1，替代 schtasks）', d1Mark, 'dll=' + dll.length + 'B');
